...
public class NewLock implements AutoCloseable {
	...
	private boolean lock(String key,String value){
		if (RedisCache.put(key, value) != null){return false;}
		else {
			lockKeys.add(key);
			return true;
		}
	...
	public ReturnResult lockKeys(String[] keys) {
		if (keys == null || keys.length == 0) {return new ReturnResult(false,"参数异常");}
		for (String key : keys) {
	...
	public void close() {
		if (lockKeys != null && lockKeys.size() > 0) {
			for (String billId : lockKeys) {RedisCache.remove(billId);}
		}
	}
}
